...
module.exports = function (app) {
	app.post('/analyzereceipt', async (req, res) => {
		...
		if (type === 'application/json') {
			...
			const result = await helper.analyze(url, type, 'receipt');
			...
			const result = await helper.analyzeFileInput(file, type, 'receipt');
			if (result.status === 202) {
				let operationId = result.headers['operation-location'];
				let temp = operationId.split('analyzeResults/');
				res.json({ 'operation-id': temp[1] });
	...
	app.get('/getreceiptdetails', async (req, res) => {
		...
		const result = await helper.getDetails(operationId, 'receipt');
